<template>
    <v-container class="d-flex justify-center mt-10">
        <v-card width="500" class="pa-6">
            ...
            <div v-else>
                <!-- Tổng tiền -->
                <v-card color="primary" variant="tonal" class="mb-4 pa-4">
                    <div class="text-subtitle-2 text-grey">Tổng tiền phải trả</div>
                ...
                <!-- Empty state -->
                <div v-if="!loading && sessions.length === 0" class="text-center py-8 text-grey">
...
const fetchData = async () => {
    ...
    try {
        ...
        const res = await api.get('/share', {
            params: { groupId, memberId, status, page: page.value, limit: 10 }
        });
